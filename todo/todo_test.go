...
func TestSaveGetDelete(t *testing.T) {
	...
	todo.Save(CTX)
	if todo.Key == nil {
		t.Errorf("Saved todo's Key should be set.")
	}
	if _, err := Get(CTX, todo.Key); err != nil {
		t.Errorf("Todo should be found! %v", err)
	}
	if err := Delete(CTX, todo.Key); err != nil {
		t.Errorf("Failed to delete todo %v: %v", todo, err)
	}
}
